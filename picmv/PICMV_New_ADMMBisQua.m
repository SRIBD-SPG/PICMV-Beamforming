function [ w, v ] = PICMV_New_ADMMBisQua( rho, mu, R, gamma, delta, H_Theta, H_Phi, M, K, Size_Theta, Size_Phi, value  )
...
scale = sum(abs(H_Theta.H))/size(H_Theta.H,2);
value = value/scale;
eps = 1e-5; % numerical precision
MaxIteration = 500; % maximun number of iteration
...
A = R+ rho/2*[HTheta,HPhi]*[HTheta,HPhi]' + rho/2*eye(M);
A = (A+A')/2;
[V, D] = eig(A);
if min(diag(D))<=0
%     A = A + abs(min(diag(D)))*eye(M);
    D = D + 1.0001*abs(min(diag(D)))*eye(M);
end
A = V*D*V';
...
%     Epsilon = epsilon;
    ...
    if no_ite > 1
        b = HTheta*(conj(lamb_theta)-rho*conj(z_theta))+...
            HPhi*(conj(lamb_phi)-rho*conj(z_phi)) + zeta - rho*v;
        %     b = b + 2*A*w - 2*la*w;
...
%     tic
    for i = 1:Size_Theta
        [z_theta(i), y_theta(i)] = CheckSubProblemTheta(rho/2, b(i), rho/2, be(i), cTheta(i), delta, d);
    end
%     toc
    %% the third block
...
%     end
    ...
    er = max([er;abs(v - vbar)]);
    % dual iteration
    er = max([er;rho*abs((w'*HPhi).' - z_phi)]);
    er = max([er;rho*abs((w'*HTheta).' - z_theta)]);
    er = max([er;rho*abs(w - v)]);
    er
end
[ w ] = ProjectionNorm( w, value*scale );
